...
export const Timeline = ({
  ...
  useEffect(() => {
    ...
    const updateHeight = () => setHeight(ref.current?.getBoundingClientRect().height ?? 0);
    updateHeight();
  ...
  return (
    <div
      className={cn("w-full bg-white font-sans dark:bg-neutral-950 md:px-10", className)}
      ref={containerRef}
    >
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              ...
              <h3 className="hidden font-bold text-neutral-500 dark:text-neutral-500 md:block md:pl-20 md:text-5xl">
                {item.title}
              </h3>
            </div>
